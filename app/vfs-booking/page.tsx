...
const upcomingCards = [
  {
    flag: "🇬🇧", country: "UK Visa", badgeColor: "bg-sky-100 text-[#0369A1] border-[#BAE6FD]",
    ...
    action: "View Slip →", actionStyle: "bg-white border border-[#BAE6FD] text-[#0ea5e9] hover:bg-sky-50",
  ...
  },
  {
    ...
    action: "Slip →", actionStyle: "bg-white border border-[#BAE6FD] text-[#0ea5e9] hover:bg-sky-50",
  ...
  },
  {
    ...
    applicant: "Awaiting VFS confirmation", applicantColor: "text-amber-600",
    action: "Check →", actionStyle: "bg-amber-50 border border-amber-200 text-amber-700 hover:bg-amber-100",
...
const statusBadge: Record<StatusType, string> = {
  ...
  upcoming:  "bg-sky-50 text-[#0369A1] border border-[#BAE6FD]",
...
const historyRows = [
  ...
  { client: "Rajesh Patil",  country: "🇨🇦 Canada PR",            centre: "Pune (Magarpatta)",         date: "18 Jun 2025 · 2:00 PM",  status: "Upcoming",  statusColor: "bg-sky-50 text-[#0369A1] border-sky-200",         action: "Reschedule →", actionStyle: "bg-amber-50 border border-amber-200 text-amber-700 hover:bg-amber-100", toast: "⏰ Reschedule requested" },
  { client: "Neha Gupta",   country: "🇪🇺 Schengen (France)",     centre: "Delhi (Shivaji Stadium)",  date: "25 Jun 2025 · 11:15 AM", status: "Pending",   statusColor: "bg-amber-50 text-amber-700 border-amber-200",     action: "Refresh →", actionStyle: "bg-white border border-[#BAE6FD] text-[#0ea5e9] hover:bg-sky-50", toast: "🔄 Check VFS status" },
];

const processSteps = [
  { emoji: "1️⃣", title: "Collect Client Docs",  desc: "Passport, photo, visa application form."                             },
  { emoji: "2️⃣", title: "Book Slot via Visara",  desc: "Fill form below — we check real-time VFS availability."              },
...
// ══════════════════════════════════════════════════════════════
export default function VFSBookingPage() {
  ...
  return (
    <div className="bg-[#F7FBFF] min-h-screen text-[#1A3347] font-sans pb-20">
      ...
      {/* ── TOP NAV ───────────────────────────────────── */}
      <nav className="bg-white border-b border-[#D4E8F5] px-7 py-4 flex items-center justify-between sticky top-0 z-40 shadow-sm">
        <div className="font-sora font-extrabold text-lg text-[#0C1A2E] flex items-center gap-2">
          <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse" />
          Visara
          <span className="text-[#0EA5E9] font-semibold text-sm ml-1">VFS Booking Services</span>
        </div>
        <div className="flex gap-3 items-center">
          <Link href="/agents">
            <button className="bg-white border border-[#BAE6FD] text-[#0EA5E9] font-bold text-xs px-4 py-2 rounded-xl hover:bg-sky-50 transition-all">Agent Login</button>
          </Link>
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#38BDF8] to-[#0284C7] flex items-center justify-center text-white font-bold text-sm shadow cursor-pointer">RM</div>
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-8">
        ...
        {/* ── HERO BANNER ───────────────────────────────── */}
        <div className="bg-gradient-to-br from-[#F0F9FF] to-white border border-[#BAE6FD] rounded-3xl p-8 md:p-10 mb-8 text-center shadow-sm relative overflow-hidden">
          {/* Subtle glow */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#0EA5E9] rounded-full opacity-[0.04] blur-3xl pointer-events-none" />
          <div className="absolute bottom-0 left-0 w-48 h-48 bg-violet-400 rounded-full opacity-[0.04] blur-3xl pointer-events-none" />

          <div className="text-5xl mb-4">🛂📅</div>
          <h1 className="font-sora font-extrabold text-3xl md:text-4xl text-[#0C1A2E] mb-3 leading-tight">
            VFS Global <span className="text-[#0EA5E9]">Booking Services</span>
          </h1>
          <p className="text-[#475569] text-sm md:text-base max-w-2xl mx-auto mb-7 leading-relaxed">
            Book biometrics, visa appointments, passport submission and collection slots for UK, Canada, Schengen, Australia and more — directly via Visara Agent Portal.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-8 mb-8">
            {[
              ...
              { value: "₹0 extra", label: "for Visara Agents" },
            ].map(s => (
              <div key={s.label} className="text-center">
                <div className="font-sora font-extrabold text-3xl text-[#0EA5E9]">{s.value}</div>
          ...
          <button
            ...
            className="bg-gradient-to-r from-[#38BDF8] to-[#0EA5E9] text-white font-bold px-7 py-3.5 rounded-2xl text-sm shadow-lg shadow-sky-200 hover:shadow-sky-300 hover:scale-[1.02] active:scale-[0.98] transition-all inline-flex items-center gap-2"
          >
          ...
          {filterChips.map(chip => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold border whitespace-nowrap shrink-0 transition-all ${
                activeChip === chip
                  ? "bg-gradient-to-r from-[#38BDF8] to-[#0EA5E9] text-white border-transparent shadow-sm"
                  : "bg-white text-[#475569] border-[#D4E8F5] hover:border-[#0EA5E9] hover:text-[#0EA5E9]"
              }`}
            >
              {chip}
            </button>
        ...
        <div className="mb-8">
          <span className="text-[11px] font-extrabold text-[#0EA5E9] uppercase tracking-widest block mb-1">Upcoming & Pending</span>
          <h2 className="font-sora font-extrabold text-xl text-[#0C1A2E] mb-5">Your Booked Appointments</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {upcomingCards.map((card, i) => (
              <div
                ...
                className="bg-white border border-[#D4E8F5] rounded-2xl p-5 shadow-sm hover:shadow-xl hover:border-[#0EA5E9] hover:-translate-y-1 transition-all duration-200 cursor-pointer group"
              >
                {/* Top row */}
                ...
                {/* Title */}
                <h3 className="font-sora font-bold text-base text-[#0C1A2E] mb-2 group-hover:text-[#0EA5E9] transition-colors leading-snug">{card.title}</h3>
                ...
                <div className="text-sm font-bold text-[#0C1A2E] flex items-center gap-1.5 mb-4">
                  <CalendarCheck className="w-3.5 h-3.5 text-[#0EA5E9] shrink-0" /> {card.date}
                </div>

                {/* Divider */}
                <div className="h-px bg-[#EEF6FD] mb-4" />
        ...
        {/* ── BOOKING FORM ──────────────────────────────── */}
        <div id="book-section" className="bg-gradient-to-br from-[#F0F9FF] to-white border-2 border-[#BAE6FD] rounded-3xl p-7 md:p-9 mb-8 shadow-sm">
          <span className="text-[11px] font-extrabold text-[#0EA5E9] uppercase tracking-widest block mb-1">Agent Booking Panel</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
            <div>
              <label className="text-[10px] font-extrabold uppercase tracking-wider text-[#94B0C4] block mb-1.5">Client Full Name <span className="text-red-400">*</span></label>
              <input
                value={clientName} onChange={e => setClientName(e.target.value)}
                className="w-full bg-white border border-[#D4E8F5] rounded-xl px-4 py-3 text-sm font-semibold text-[#0C1A2E] outline-none focus:border-[#0EA5E9] focus:ring-2 focus:ring-sky-100 transition-all placeholder:text-[#C4D9E8]"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select value={country} onChange={e => setCountry(e.target.value)}
                  className="w-full bg-white border border-[#D4E8F5] rounded-xl pl-10 pr-4 py-3 text-sm font-semibold text-[#0C1A2E] outline-none focus:border-[#0EA5E9] appearance-none cursor-pointer transition-all">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
            ...
            <div>
              <label className="text-[10px] font-extrabold uppercase tracking-wider text-[#94B0C4] block mb-1.5">Preferred Date <span className="text-red-400">*</span></label>
              <div className="relative">
                ...
                <input type="date" value={prefDate} onChange={e => setPrefDate(e.target.value)}
                  className="w-full bg-white border border-[#D4E8F5] rounded-xl pl-10 pr-4 py-3 text-sm font-semibold text-[#0C1A2E] outline-none focus:border-[#0EA5E9] transition-all" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
            ...
            <div>
              ...
              <input value={agentNote} onChange={e => setAgentNote(e.target.value)}
                className="w-full bg-white border border-[#D4E8F5] rounded-xl px-4 py-3 text-sm font-semibold text-[#0C1A2E] outline-none focus:border-[#0EA5E9] transition-all placeholder:text-[#C4D9E8]"
                placeholder="Any special request for client?" />
            </div>
          </div>

          {/* Tip box */}
          <div className="bg-sky-50 border-l-4 border-[#0EA5E9] rounded-r-2xl px-5 py-4 mb-6 flex gap-3 items-start">
            <Lightbulb className="w-5 h-5 text-[#0EA5E9] shrink-0 mt-0.5" />
          ...
          <div className="flex gap-3 justify-end flex-wrap">
            <button onClick={handleClear}
              className="bg-white border border-[#BAE6FD] text-[#0EA5E9] font-bold text-sm px-6 py-3 rounded-2xl hover:bg-sky-50 transition-all active:scale-[0.97]">
              Clear
            </button>
            <button onClick={handleBook}
              className="bg-gradient-to-r from-[#38BDF8] to-[#0EA5E9] text-white font-bold text-sm px-7 py-3 rounded-2xl shadow-md shadow-sky-200 hover:shadow-sky-300 hover:scale-[1.01] active:scale-[0.98] transition-all flex items-center gap-2">
        ...
        <div className="mb-8">
          <span className="text-[11px] font-extrabold text-[#0EA5E9] uppercase tracking-widest block mb-1">Booking History</span>
          <h2 className="font-sora font-extrabold text-xl text-[#0C1A2E] mb-5">Recent Appointments Booked by You</h2>

          <div className="bg-white border border-[#D4E8F5] rounded-2xl overflow-hidden shadow-sm">
            {/* Table header */}
            <div className="hidden md:grid grid-cols-12 bg-[#EEF6FD] border-b border-[#D4E8F5] px-5 py-3 text-[10px] font-extrabold text-[#94B0C4] uppercase tracking-wider">
            ...
            {historyRows.map((row, i) => (
              <div key={i} className={`grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-0 px-5 py-4 border-b border-[#EEF6FD] last:border-0 items-center text-sm ${i % 2 === 1 ? "bg-[#F7FBFF]" : "bg-white"} hover:bg-sky-50 transition-colors`}>
        ...
        {/* ── VFS PROCESS GUIDE ─────────────────────────── */}
        <div className="bg-white border border-[#D4E8F5] rounded-3xl p-7 mb-8 shadow-sm">
          <span className="text-[11px] font-extrabold text-[#0EA5E9] uppercase tracking-widest block mb-1">VFS Process Guide</span>
          <h2 className="font-sora font-extrabold text-xl text-[#0C1A2E] mb-7">Step-by-Step for Visara Agents</h2>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
            {processSteps.map((step, i) => (
              <div key={i} className="text-center group">
                <div className="w-14 h-14 bg-[#E0F2FE] rounded-2xl flex items-center justify-center text-2xl mx-auto mb-4 group-hover:scale-110 transition-transform shadow-sm">
                  {step.emoji}
                </div>
        ...
        {/* ── FAQ / QUICK LINKS ─────────────────────────── */}
        <div className="flex flex-wrap items-center justify-between gap-4 py-4 border-t border-[#D4E8F5]">
          <button onClick={() => showToast("🔗 VFS Official website")}
            className="flex items-center gap-1.5 text-xs text-[#0EA5E9] font-semibold hover:underline">
          ...
          <button onClick={() => showToast("Mon–Sat, 9 AM – 5:30 PM")}
            className="flex items-center gap-1.5 text-xs text-[#475569] font-semibold hover:text-[#0EA5E9] transition-colors">
