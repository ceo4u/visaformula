...
export function UniversityCard({ uni }: { uni: any }) {
    return (
        <a href="/university/1" className="block h-full cursor-pointer">
            <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:border-yellow-200 transition-all duration-300 flex flex-col h-full group">
                ...
                <div className="p-5 flex flex-col flex-1 gap-2.5">
                    <div>
                        <h3 className="font-bold text-[#1a1a2e] text-lg leading-tight group-hover:text-[#f59e0b] transition-colors">{uni.name}</h3>
                    ...
                    <div className="flex justify-between items-end mt-auto pt-3 border-t border-gray-100">
                        ...
                        <button className="bg-white border-2 border-[#f59e0b] text-[#f59e0b] px-4 py-2 rounded-xl text-sm font-bold hover:bg-[#f59e0b] hover:text-white transition-all">
                            View Programs
                        </button>
                    </div>
                </div>
