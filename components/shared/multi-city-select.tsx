...
export function MultiCitySelect({ selected, selectedCities, onChange, placeholder }: MultiCityProps) {
    const cities = selected ?? selectedCities ?? [];
    const [input, setInput] = useState("");
    const [showSuggestions, setShowSuggestions] = useState(false);

    const filtered = popularCities.filter(c => c.toLowerCase().includes(input.toLowerCase()) && !cities.includes(c));
        ...
        <div className="relative">
            ...
            <div className="flex flex-wrap gap-1.5 p-2 bg-sky-50/50 border border-sky-100 rounded-xl min-h-[48px] items-center">
                ...
                <input
                    ...
                    placeholder={placeholder ?? (cities.length ? "Add more..." : "Type a city...")}
