...
<button onClick={() => setShowMobileFilters(false)}><X className="w-5 h-5 text-gray-400" /></button>
                ...
                <section className="flex-1">
                    ...
                    {/* Expert / Consultant Google Login Fast-Track Bar */}
                    <div className="bg-gradient-to-r from-slate-900 via-teal-950 to-slate-900 border border-teal-500/30 rounded-3xl p-4 sm:p-5 shadow-xl mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-white">
                        <div className="flex items-center gap-3.5 w-full sm:w-auto">
                            <div className="w-10 h-10 rounded-2xl bg-teal-500/20 border border-teal-400/30 flex items-center justify-center text-[#00a896] shrink-0 font-bold">
                                <Sparkles className="w-5 h-5 text-teal-300" />
                            </div>
                            <div>
                                <div className="flex items-center gap-2">
                                    <h3 className="text-sm font-extrabold text-white leading-tight">Are you an Immigration Consultant or Lawyer?</h3>
                                    <span className="bg-teal-500/20 text-teal-300 border border-teal-500/40 text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider hidden md:inline-block">Consultant Portal</span>
                                </div>
                                <p className="text-xs text-slate-300 mt-0.5">Sign in with Google to access your dashboard, list services & manage leads.</p>
                            </div>
                        </div>

                        <div className="flex items-center gap-2.5 w-full sm:w-auto shrink-0 justify-end">
                            <button
                                onClick={handleGoogleExpertLogin}
                                disabled={googleLoading}
                                className="w-full sm:w-auto px-4 py-2.5 bg-white hover:bg-slate-100 text-slate-900 rounded-xl font-bold text-xs flex items-center justify-center gap-2 shadow-md transition-all cursor-pointer active:scale-95 whitespace-nowrap"
                            >
                                {googleLoading ? (
                                    ...
                                    <>
                                        ...
                                        <span>Expert Login with Google</span>
                                    </>
                                )}
                            </button>
                            <a
                                href="/signup/expert"
                                className="hidden lg:flex items-center gap-1 text-xs font-bold text-teal-300 hover:text-white px-3 py-2 rounded-xl transition-colors whitespace-nowrap"
                            >
                                <span>Register</span>
