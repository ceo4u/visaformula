...
export function VisaCountryResultPortal({ 
  ...
  const handleProceedToRazorpay = (e: React.FormEvent) => {
    ...
    // Redirect to Razorpay Payment Checkout page
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-[#00A86B] selection:text-white relative">
      ...
      {/* ── APPLICATION DETAILS POP-UP MODAL (LIKE LOGIN/SEEKER REGISTRATION) ── */}
      {showApplicationModal && (
        ...
        >
          <div className="bg-white rounded-[32px] border border-slate-200 shadow-2xl max-w-xl w-full p-6 sm:p-8 text-left relative my-auto max-h-[92vh] overflow-y-auto space-y-6 font-sans">
            
            {/* Top Close Button */}
            <button
              type="button"
              onClick={() => setShowApplicationModal(false)}
              className="absolute top-5 right-5 w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-900 flex items-center justify-center transition-all cursor-pointer shadow-2xs border border-slate-200"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="space-y-1 pr-10">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-[#00A86B] text-xs font-black border border-emerald-200 mb-1">
            ...
            {/* Order Summary Strip */}
            <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-4 flex items-center justify-between text-xs">
              <div className="space-y-0.5">
                <span className="font-extrabold text-slate-900 block">
                  {countryName} • {currentVariant.label}
                </span>
                <span className="text-slate-500 font-medium">
                  {travellerCount} {travellerCount === 1 ? 'Applicant' : 'Applicants'} • Guaranteed {guaranteedDate}
                </span>
              </div>
